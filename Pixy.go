...
func ExampleHandler(serverip string) {
	split_ip := strings.Split(serverip, ".")
	oct1, _ := strconv.Atoi(split_ip[0])
	oct2, _ := strconv.Atoi(split_ip[0])
	oct3, _ := strconv.Atoi(split_ip[0])
	oct4, _ := strconv.Atoi(split_ip[0])
	...
	handler := &DHCPHandler{
		...
		options: dhcp.Options{
			...
			dhcp.OptionBootFileName:     []byte("test"),
		},
	}
...
func startdhcp() {
	go ExampleHandler("192.168.4.10")
}
