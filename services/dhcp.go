...
import (
	...
	conn4 "github.com/krolaw/dhcp4/conn"
	"math/rand"
	"net"
	"strconv"
	"strings"
...
// DO NOT TOUCH THE DHCPHandler functions. It works, no touchy
...
func DhcpProcess(serverip string, iface string) {
	splitIp := strings.Split(serverip, ".")
	oct1, _ := strconv.Atoi(splitIp[0])
	oct2, _ := strconv.Atoi(splitIp[1])
	oct3, _ := strconv.Atoi(splitIp[2])
	oct4, _ := strconv.Atoi(splitIp[3])
	serverIP := net.IP{byte(oct1), byte(oct2), byte(oct3), byte(oct4)}

	handler := &DHCPHandler{
		ip:            serverIP,
		leaseDuration: 2 * time.Hour,
		start:         net.IP{byte(oct1), byte(oct2), byte(oct3), byte(oct4 + 1)},
		leaseRange:    50,
		...
		options: dhcp.Options{
			...
			dhcp.OptionPxelinuxConfigfile: []byte("default"),
			dhcp.OptionBootFileName: []byte("pxelinux.0"), // Defaulting to point to pxelinux since it's included in the repo
		},
	}

	l, err := conn4.NewUDP4FilterListener(iface, "0.0.0.0:67")
	if err != nil {
	} else {
		defer l.Close()
		dhcpErr := dhcp.Serve(l, handler).Error()
		if dhcpErr != "" {
		}
	}
}